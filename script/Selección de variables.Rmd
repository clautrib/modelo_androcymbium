...
```{r}
library(readxl)
presencia <- read_excel("data/presencia_ausencia/df_presencia.xlsx")
...
library(dplyr)
dfmodelo  <- mutate_at(dfpresencia, "layer", ~replace(., is.na(.), 0)) #cambiar los NAs por 0 en las columnas seleccionadas 

library(car)
dfmodelo <- dplyr::select(dfmodelo, -x, -y) #eliminar dos columnas con el paquete dplyr
head(dfmodelo)
modelo <- glm(layer~ DS_URBAN + HIDRO_ICT+ TP_RSD_A+CLI_TMNI+TP_ES_OE
+HIDRO_ITH+ DS_AGUA+ DS_CULTI+ HIDRO_ACUM+MT_ARBOL+FR_PASTO+ FR_VIASC , family=binomial, data=dfmodelo) #glm binomial binaria xq la variable dependiente es de tipo 1/0
