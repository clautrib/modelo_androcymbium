...
# Cross Validation 
spatialblocks <- readRDS("data/cv/lista_spatialblocks.rds")

cv_biomod_block_tables <- lapply(spatialblocks, function(block) {
  # Verificar si biomod_table existe en el elemento actual
  if ("biomod_table" %in% names(block)) {
    return(block$biomod_table)
  } else {return(NULL)
  }
})
...
modelling_options <-BIOMOD_ModelingOptions(
 ...
 a <- cv_biomod_block_tables[["PA1"]]
matriz_a_dataframe <- function(mat) {
  data.frame(t(mat))
}
cv_biomod_block_df <- lapply(cv_biomod_block_tables[[]], matriz_a_dataframe)
